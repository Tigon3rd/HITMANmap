		...
		// $.getScript("files/js/mapdata-velen.js").done(function(script, textStatus) {
			...
			// $.i18n.loadNamespace('s', function() {
				// $.getScript("files/js/mapdata-skellige.js").done(function(script, textStatus) {
					// $(document).trigger('loadMapdata');
					// $(document).unbind('loadMapdata');
					$.i18n.loadNamespace('a', function() {
						$.getScript("files/js/mapdata-a.js").done(function(script, textStatus) {
							$(document).trigger('loadMapdata');
							$(document).unbind('loadMapdata');
...
var processData = function(data) {
	...
	$.each(data, function(markerType,markers) {
		$.each(markers, function(index,marker) {
			//var link = window.location.href.replace(window.location.hash, '')+'c/#4/'+marker.coords[0][0]+'/'+marker.coords[0][1]+'/m='+marker.coords[0][0]+','+marker.coords[0][1];
			var link = window.location.href.replace(window.location.hash, '')+mapKey+'/#6/'+marker.coords[0][0]+'/'+marker.coords[0][1]+'/m='+marker.coords[0][0]+','+marker.coords[0][1];
